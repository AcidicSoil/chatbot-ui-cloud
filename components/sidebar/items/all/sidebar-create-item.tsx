...
export const SidebarCreateItem: FC<SidebarCreateItemProps> = ({
  ...
  const createFunctions = {
    ...
    assistants: async (
      ...
      let updatedAssistant: Tables<"assistants"> = createdAssistant
      ...
      const assistantFiles = (files as Tables<"files">[]).map(file => ({
        user_id: rest.user_id,
        assistant_id: createdAssistant.id,
        file_id: file.id
      }))

      const assistantCollections = (collections as Tables<"collections">[]).map(
        collection => ({
          user_id: rest.user_id,
          assistant_id: createdAssistant.id,
      ...
      const assistantTools = (tools as Tables<"tools">[]).map(tool => ({
        user_id: rest.user_id,
        assistant_id: createdAssistant.id,
        tool_id: tool.id
      }))
